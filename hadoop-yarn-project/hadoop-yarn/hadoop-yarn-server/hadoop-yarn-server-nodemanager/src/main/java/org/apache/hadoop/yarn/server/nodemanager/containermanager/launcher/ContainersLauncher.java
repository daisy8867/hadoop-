...
public class ContainersLauncher extends AbstractService
  ...
  public void handle(ContainersLauncherEvent event) {
    ...
    final ContainerId containerId = container.getContainerId();
      ...
      case LAUNCH_CONTAINER:
		...
		/*if(container.getResource().getGPUCores() > 0){
		  new Thread(new Runnable() {
			  ...
			  public void run() {
			      try{
     			      if(container.getMetrics().lock.tryLock(20000L, TimeUnit.MILLISECONDS))
     				  {
     				      try{
                             ...
                             Path containerLogDir =
     				         ...
     				         LOG.info(" containerLogDir: " + containerLogDir);
     				         Thread.sleep(19999);
                              container.getMetrics().lock.unlock();
     				         LOG.info(" lock released by" + containerId);
     		              }
     				  }
			      }catch (Exception e){
